...
Model: whatever VISION_* points at in your .env. Left blank, pictures are read
by the same model and model runner as the "why it matched" sentences.
"""
...
# Fixed on purpose - every image is asked the same thing, so results stay
# consistent.
VISION_INSTRUCTION = (
...
# How long to wait for one picture. The same generous limit as the text model,
# because a model on an ordinary computer can take a while per picture.
REQUEST_TIMEOUT_SECONDS = config.LLM_TIMEOUT_SECONDS

# No pause between pictures: a model runner on your own computer has no usage
# limit to stay under.
PAUSE_BETWEEN_IMAGES_SECONDS = 0
# If the model runner says it is busy, wait a little and try once more.
RATE_LIMIT_WAIT_SECONDS = 15
# Deliberately few. If the model is failing, waiting longer does not help - it
# just leaves you staring at a program that looks frozen. Better to fail in
# seconds and say so, then try again later.
MAX_ATTEMPTS = 2
...
def _suggested_wait(message: str) -> float | None:
    """
    A busy service sometimes says how long to wait, e.g. "Please try again in
    6.79s" or "try again in 1m30s". Honouring that beats guessing.
    """
...
# If the model fails on this many pictures in a row, it is almost certainly not
# working right now (for example, the model runner was closed). Carrying on
# would mean minutes of waiting per picture for nothing, so we stop asking for
# the rest of this run and let the indexer record them as still to do.
...
def describe_image(source) -> str:
    ...
    if giving_up():
        raise VisionError(
            "the picture model keeps failing - skipping the remaining pictures "
            "this run. Processing the files again later picks them up"
        )
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            # The model is briefly busy or failing. That often clears up on its
            # own, so wait and try again.
            busy = "over capacity" in message.lower()
            if response.status_code in (429, 500, 502, 503) or busy:
                # Say what actually happened. A 429 is a usage limit on an
                # online service; a 500-type error is the model itself failing -
                # on your own computer, usually a model runner that needs
                # restarting or a model that is not loaded.
                if busy:
                    last_problem = "the picture model was busy"
                elif response.status_code == 429:
                    last_problem = "hit the picture service's usage limit"
                else:
                    last_problem = (f"the picture model returned an error "
                                    f"({response.status_code}: {message[:80]})")
