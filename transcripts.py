...
def fetch_transcript(url: str) -> dict:
    ...
    # Each caption line is a short fragment with the moment it is spoken. Keep
    # every word paired with that moment, so an excerpt can later say where in
    # the video it starts.
    timed_words = []
    pieces = 0
    for snippet in fetched:
        # Captions often contain [Music], [Applause] and similar stage directions.
        line = re.sub(r"\[[A-Za-z ]{1,20}\]", " ", snippet.text or "")
        words = line.split()
        if not words:
            continue
        pieces += 1
        start = int(getattr(snippet, "start", 0) or 0)
        timed_words.extend((word, start) for word in words)
    ...
    return {
        "video_id": identifier,
        "text": " ".join(word for word, _ in timed_words),
        # [(word, second it is spoken at), ...]
...
def split_into_timed_parts(timed_words: list[tuple[str, int]],
                           words_per_part: int = WORDS_PER_PART,
                           overlap: int = OVERLAP_WORDS) -> list[dict]:
    """
    The same cut as split_into_parts, but each part also says the second in
    the video where its first word is spoken.

    Returns [{"text": ..., "start_seconds": ...}, ...].
    """
    if not timed_words:
        return []
    ...
    step = max(words_per_part - overlap, 1)
    parts = []
    for start in range(0, len(timed_words), step):
        piece = timed_words[start:start + words_per_part]
        if not piece:
            break
        parts.append({
            "text": " ".join(word for word, _ in piece),
            "start_seconds": piece[0][1],
        })
        if start + words_per_part >= len(timed_words):
            break
    return parts


def format_time(seconds: int) -> str:
    """412 -> "6:52", 3725 -> "1:02:05" - the way YouTube writes times."""
