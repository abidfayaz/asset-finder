...
def web_link(result: dict) -> str:
    ...
    # A video excerpt with a known start time links straight to that moment.
    # The link is rebuilt in the standard form so "&t=" always follows a "?",
    # whichever link style the spreadsheet used.
    start = result.get("start_seconds")
    if result.get("type") == "youtube" and start is not None:
        import transcripts
        try:
            identifier = transcripts.video_id(cleaned)
        except transcripts.TranscriptUnavailable:
            return cleaned
        return f"https://www.youtube.com/watch?v={identifier}&t={int(start)}s"
    return cleaned


def display_location(result: dict) -> str:
    """
    What to show as the result's location: the full path, which you can paste
    into File Explorer to open the file. A video shows its web address.
    """
...
def search(query: str, top_k: int = None, collection=None, model=None) -> list[dict]:
    ...
    # An index can only be searched with the model that built it. Checking
    # here turns a baffling database error into an instruction.
    indexer.check_index_matches_model()

    # encode_query, not encode_documents: some models (mxbai) want what you
    # typed prefixed with a short instruction that the stored text never had.
